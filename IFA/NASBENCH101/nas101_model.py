######################################################################################################################
# This code is based on the Neural Architecture Search Without Training code, which is licensed under the MIT License.
# A modification was done to the original version, by fetching architecture configuration from IFA.Utils.GlobalInformation.
# Original version can be found at https://github.com/BayesWatch/nas-without-training
# You can find a copy of the MIT License at https://opensource.org/license/mit/
#####################################################################################################################
...
from IFA.NASBENCH101.base_ops import *
import torch
...
class Network(nn.Module):
    def __init__(self, spec):
        ...
        for stack_num in range(GlobalInformation.Params.config['num_stacks']):
            if stack_num > 0:
            ...
            for module_num in range(GlobalInformation.Params.config['num_modules_per_stack']):
                cell = Cell(spec, in_channels, out_channels)
                self.layers.append(cell)
                in_channels = out_channels

        self.classifier = nn.Linear(out_channels, GlobalInformation.Params.nb_classes)
